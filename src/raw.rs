...
/// ```toml
/// [dependencies]
/// serde_json_untagged = { version = "1.0", features = ["raw_value"] }
/// ```
///
/// # Example
///
...
/// use serde_json_untagged::{Result, value::RawValue};
///
/// #[derive(Deserialize)]
/// struct Input<'a> {
///     code: u32,
...
/// //
/// // This could be done equivalently using serde_json_untagged::Value as the type for
...
///     let input: Input = serde_json_untagged::from_str(input)?;
///
///     let output = Output {
///         info: (input.code, input.payload),
///     };
///
///     serde_json_untagged::to_string(&output)
/// }
///
/// fn main() -> Result<()> {
...
/// [`serde_json_untagged::from_str`] and [`serde_json_untagged::from_slice`] which support
/// borrowing from the input data without memory allocation.
///
/// When deserializing through [`serde_json_untagged::from_reader`] you will need to use
...
///
/// [`serde_json_untagged::from_str`]: crate::from_str
/// [`serde_json_untagged::from_slice`]: crate::from_slice
/// [`serde_json_untagged::from_reader`]: crate::from_reader
///
/// ```
/// # use serde::Deserialize;
/// # use serde_json_untagged::value::RawValue;
...
impl RawValue {
    ...
    ///
    /// This function is equivalent to `serde_json_untagged::from_str::<Box<RawValue>>`
    ...
    ///     let response: Response = serde_json_untagged::from_str(input)?;
    ///
...
/// // Local crate
/// use serde::Serialize;
/// use serde_json_untagged::value::{to_raw_value, RawValue};
///
/// #[derive(Serialize)]
/// struct MyExtraData {
///     a: u32,
...
/// };
/// # assert_eq!(
/// #     serde_json_untagged::to_value(my_thing).unwrap(),
/// #     serde_json_untagged::json!({
/// #         "foo": "FooVal",
/// #         "bar": null,
/// #         "extra_data": { "a": 1, "b": 2 }
/// #     })
...
///
/// println!("{}", serde_json_untagged::value::to_raw_value(&map).unwrap_err());
/// ```
#[cfg_attr(docsrs, doc(cfg(feature = "raw_value")))]
...
pub const TOKEN: &str = "$serde_json_untagged::private::RawValue";
