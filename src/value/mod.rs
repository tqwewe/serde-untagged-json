...
//!
//! ```
//! use serde_json_untagged::{json, Value, Error};
//!
...
///
/// See the [`serde_json_untagged::value` module documentation](self) for usage examples.
...
pub enum Value {
    ...
    /// feature of serde_json_untagged to use IndexMap instead, which preserves
...
impl Value {
    ...
    ///
    /// ```
    /// # use serde_json_untagged::{json, Number};
    /// #
    ...
    /// fn main() {
    ///     let s = r#"{"x": 1.0, "y": 2.0}"#;
    ///     let mut value: Value = serde_json_untagged::from_str(s).unwrap();
    ///
...
/// }
///
/// # fn try_main() -> Result<(), serde_json_untagged::Error> {
/// let data = r#" { "level": 42 } "#;
/// let s: Settings = serde_json_untagged::from_str(data)?;
///
...
/// Convert a `T` into `serde_json_untagged::Value` which is an enum that can represent
/// any valid JSON data.
///
/// # Example
///
...
///     };
///
///     // The type of `expected` is `serde_json_untagged::Value`
///     let expected = json!({
///         "fingerprint": "0xF9BA143B95FF6D82",
///         "location": "Menlo Park, CA",
///     });
///
///     let v = serde_json_untagged::to_value(u).unwrap();
...
///
///     println!("{}", serde_json_untagged::to_value(map).unwrap_err());
/// }
/// ```
...
/// Interpret a `serde_json_untagged::Value` as an instance of type `T`.
///
/// # Example
///
/// ```
...
///
/// fn main() {
///     // The type of `j` is `serde_json_untagged::Value`
///     let j = json!({
///         "fingerprint": "0xF9BA143B95FF6D82",
///         "location": "Menlo Park, CA"
///     });
///
///     let u: User = serde_json_untagged::from_value(j).unwrap();
///     println!("{:#?}", u);
/// }
/// ```
///
